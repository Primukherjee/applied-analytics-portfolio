...
## Structural Concentration Metrics (Added Analysis)

I also quantified how concentrated revenue actually is and I found: 
- Total revenue in dataset: ~10,666,684
- Revenue from United Kingdom: ~9,025,222
- UK share of total revenue: ~84.6%
I calculated the Herfindahl–Hirschman Index (HHI) in order to mmeasure structural concentration more exactly.
- HHI = 0.718
* We know that an HHI above 0.25 indicates high concentration in economic analysis. So a 0.718 suggests geographic dependency on a significant level. Exposure is measurable. 
...
- Geographic concentration shows 85% revenue from one single country and secondary markets each contribute less than 3% that basically shows that it is centralization and not diversification. 
- Customer 14646 alone generated ~280,206, Customer 18102 generated ~259,657-->customer inequality
- Dual customer model: loss of a frequent buyer leads to less cash flow while the loss of a high value client leads to reduced margin spikes. 
- There was a seasonal volatility: Revenue dipped to ~520,000 in February while revenue peaked at ~1.5 million in November. This shows a 3x monthly swing.
...
## Revenue Fragility Simulation (Top Customer Shock Test)
Additionally, I simulated what exactly would happen if the top clients were all removed in a sequential manner in order to understand sensitivity. So I calculated the revenue per customer and sorting from highest to lowest while removing them one by one. 
- Remove top 1 customer: Revenue drops to ~96.86% of original
- Remove top 5 customers: Revenue drops to ~88.25%
- Remove top 10 customers: Revenue drops to ~82.74%
This implies that when we lose 10 customers, total revenue drops by more than 17% even without touching the geographical concentration. So, it is clear from this that the stability of revenue depends on a small portion of high-impact clients. 

## What Should the Business Focus on Next?
This is what the business should focus on based on the pattersn observed in the analysis: 
- Geographic diversification is a big one: Business should explore expansion strategies in secondary markets like Netherlands, Germany, and France
- Business should invest on tier-based monitoring of customers and explore other retention strategies specifically for high-impact customers.
- We saw that revenue rises around November and drops towards the early months signalling seasonality. Business should focus more on inventory, staffing, and other marketing strategies to manage these yearly fluctuations.
- Perhaps a more controlled returns analysis might reveal more number of opperational cost patterns.
- Business needs to perhaps monitor the HHI metric quarterly in order to better track the diversification progress.
- Another thing to implement is to differentiate between frequency-driven revenue and high-ticket revenue across different structural revenue streams.
- There should be a cash flow smoothig strategy implemented.
- KPIs should be diversified for example, tracking HHI score over time.
